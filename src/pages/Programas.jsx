...
const Programas = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.fechaInicio && formData.fechaCierre && formData.fechaInicio > formData.fechaCierre) {
      errors.fechaCierre = 'La fecha de cierre debe ser posterior al inicio';
    }
    if (formData.montoApoyo && isNaN(parseFloat(formData.montoApoyo))) {
      errors.montoApoyo = 'Ingresa un monto válido';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dataToSend = {
        ...rest,
        categoriaNegocioId: formData.categoriaNegocioId
          ? parseInt(formData.categoriaNegocioId)
          : null,
  ...
  const handleToggleActivo = async (programa) => {
    ...
    if (window.confirm(`¿Estás seguro de ${accion} "${programa.nombre}"?`)) {
      try {
        await programasService.toggleActivo(programa.id);
        toast.success(`Programa ${accion === 'desactivar' ? 'desactivado' : 'activado'} exitosamente`);
        cargarDatos();
      } catch {
  ...
  const getTipoColor = (tipo) => {
    const map = {
      Nacional: { bg: '#EEF4FF', color: 'var(--capyme-blue-mid)' },
      Estatal: { bg: '#F0FDF4', color: '#16A34A' },
      Municipal: { bg: '#FFF7ED', color: '#C2410C' },
    };
    ...
    return (
      <Layout>
        ...
        >
          ...
          />
          <span style={{ fontSize: '14px', color: 'var(--gray-500)', fontFamily: "'DM Sans', sans-serif" }}>
            Cargando programas...
          </span>
  ...
  return (
    <Layout>
      <style>{`
        ...
        .prog-card:hover {
          box-shadow: 0 8px 24px rgba(31,78,158,0.10);
      ...
      <div style={{ padding: '0 0 40px' }}>
        ...
        >
          {[
            ...
            {
              label: 'Con Monto',
              value: programas.filter((p) => p.montoApoyo).length,
              color: '#C2410C',
              bg: '#FFF7ED',
            },
          ].map((stat) => (
        ...
        {programasFiltrados.length === 0 ? (
          ...
          >
            ...
            <p style={{ fontSize: '15px', fontWeight: 600, color: 'var(--gray-700)', margin: '0 0 6px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              No se encontraron programas
            </p>
            <p style={{ fontSize: '13px', color: 'var(--gray-400)', margin: 0, fontFamily: "'DM Sans', sans-serif" }}>
              Intenta con otros filtros o crea un nuevo programa
            </p>
          </div>
        ) : viewMode === 'grid' ? (
          /* ── GRID VIEW ── */
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    opacity: programa.activo ? 1 : 0.65,
                  }}
                >
                  {/* Card top accent */}
                  <div
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                          ...
                          <span
                            style={{
                              ...
                              fontSize: '11px',
                              fontWeight: 700,
                              background: programa.activo ? '#ECFDF5' : '#FEF2F2',
                              color: programa.activo ? '#065F46' : '#DC2626',
                              fontFamily: "'Plus Jakarta Sans', sans-serif",
                            }}
                          >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginBottom: '16px' }}>
                      {programa.montoApoyo && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                          ...
                          <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--gray-800)', fontFamily: "'DM Sans', sans-serif" }}>
                    ...
                    >
                      <div style={{ marginLeft: 'auto', display: 'flex', gap: '4px' }}>
                        ...
                        {currentUser.rol === 'admin' && !programa.activo && (
                          <button
                            onClick={() => handleToggleActivo(programa)}
                            title="Activar"
                            style={{
                              width: '34px', height: '34px', border: 'none',
                              borderRadius: 'var(--radius-sm)', background: 'transparent',
              ...
              <tbody>
                {programasFiltrados.map((programa) => {
                  ...
                  return (
                    <tr
                      key={programa.id}
                      onMouseEnter={() => setHoveredRow(programa.id)}
                      onMouseLeave={() => setHoveredRow(null)}
                      style={{
                        borderBottom: '1px solid var(--gray-100)',
                        background: hoveredRow === programa.id ? 'var(--gray-50)' : '#fff',
                    ...
                    >
                      <td style={{ padding: '13px 16px', maxWidth: '260px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          <div>
                            <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--gray-900)', fontFamily: "'DM Sans', sans-serif" }}>
                              {programa.nombre}
                            </div>
                            {programa.descripcion && (
                              <div
                      ...
                      <td style={{ padding: '13px 16px' }}>
                        {programa.tipoPrograma ? (
                          ...
                          <span style={{ fontSize: '13px', color: 'var(--gray-300)' }}>—</span>
                        )}
                      </td>
                      <td style={{ padding: '13px 16px', fontSize: '13px', color: 'var(--gray-600)', fontFamily: "'DM Sans', sans-serif" }}>
                        {programa.categoria?.nombre || <span style={{ color: 'var(--gray-300)' }}>—</span>}
                      </td>
                      <td style={{ padding: '13px 16px', fontSize: '13px', fontWeight: 600, color: 'var(--gray-800)', fontFamily: "'DM Sans', sans-serif" }}>
                        {formatCurrency(programa.montoApoyo) || <span style={{ color: 'var(--gray-300)', fontWeight: 400 }}>—</span>}
                      </td>
                      <td style={{ padding: '13px 16px' }}>
                        {programa.fechaInicio ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                            <Clock style={{ width: '12px', height: '12px', color: 'var(--gray-400)' }} />
                            <span style={{ fontSize: '12px', color: 'var(--gray-500)', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}>
              ...
              <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={labelStyle}>
                    Nombre del Programa <span style={{ color: '#EF4444' }}>*</span>
                  </label>
                  <input
                    name="nombre"
                    type="text"
                    ...
                    placeholder="Ej. Fondo PyME 2025"
                    ...
                    <div style={{ position: 'relative' }}>
                      <select
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({
                            ...prev,
                            categoriaNegocioId: e.target.value ? parseInt(e.target.value) : null,
                          }))
                        }
                        style={selectStyle}
                      >
                ...
                <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      style={{ ...inputBaseStyle, ...(formErrors.fechaCierre ? inputErrorStyle : {}) }}
                    />
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        placeholder="0.00"
                        style={{ ...inputWithIconStyle, ...(formErrors.montoApoyo ? inputErrorStyle : {}) }}
                      />
                    </div>
