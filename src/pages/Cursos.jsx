...
const Cursos = () => {
  ...
  const handleDeclinarPago = (pago) => {
    showConfirm({
      variant:'danger', title:'Declinar inscripción', subtitle:'El cliente será notificado',
  ...
  const validateForm = () => {
    ...
    if(formData.duracionHoras!==''&&isNaN(parseInt(formData.duracionHoras)))
      errors.duracionHoras='Ingresa un número válido';
  ...
  const getEstadoStyle   =(e)=>({
    inscrito:   {bg:'#EEF4FF',color:'var(--capyme-blue-mid)'},
    en_curso:   {bg:'#FFF7ED',color:'#C2410C'},
    completado: {bg:'#ECFDF5',color:'#065F46'},
    abandonado: {bg:'#FEF2F2',color:'#DC2626'},
        ...
        <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'28px',flexWrap:'wrap',gap:'16px' }}>
          ...
          <div style={{ display:'flex',gap:'10px',flexWrap:'wrap' }}>
            {currentUser.rol==='admin'&&(
              <button onClick={cargarPagosPendientes} disabled={loadingPagos}
                ...
                <CreditCard style={{ width:'16px',height:'16px' }} />Pagos pendientes
              </button>
            )}
        ...
        <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.42)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'20px' }}>
          <div className="curso-modal" onClick={e=>e.stopPropagation()} style={{ background:'#fff',borderRadius:'var(--radius-lg)',width:'100%',maxWidth:'720px',maxHeight:'92vh',display:'flex',flexDirection:'column',boxShadow:'0 24px 64px rgba(0,0,0,0.18)',overflow:'hidden' }}>
              ...
              <div style={{ marginTop:'24px' }}>
                ...
                <div style={{ marginTop:'14px',display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'14px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:'11px',color:'var(--gray-400)',margin:'4px 0 0',fontFamily:"'DM Sans', sans-serif" }}>0 = gratuito · Pago vía Mercado Pago</p>
                  </div>
                </div>
              </div>
              <div style={{ marginTop:'24px' }}>
            ...
            <div style={{ display:'flex',justifyContent:'flex-end',gap:'10px',padding:'16px 24px',background:'var(--gray-50)',borderTop:'1px solid var(--border)',flexShrink:0 }}>
              <button onClick={handleCloseModal} disabled={submitting} style={{ padding:'9px 18px',border:'1px solid var(--border)',borderRadius:'var(--radius-md)',background:'#fff',color:'var(--gray-700)',fontSize:'14px',fontWeight:600,fontFamily:"'DM Sans', sans-serif",cursor:'pointer' }} onMouseEnter={e=>e.currentTarget.style.background='var(--gray-100)'} onMouseLeave={e=>e.currentTarget.style.background='#fff'}>Cancelar</button>
            ...
            <div style={{ overflowY:'auto',flex:1 }}>
              {inscritos.length===0?(
                <div style={{ padding:'60px 20px',textAlign:'center' }}>
                  ...
                  <p style={{ fontSize:'14px',fontWeight:600,color:'var(--gray-600)',margin:0,fontFamily:"'Plus Jakarta Sans', sans-serif" }}>Sin inscritos</p>
                </div>
              ):(
                <table style={{ width:'100%',borderCollapse:'collapse' }}>
                  <thead>
                  ...
                  <tbody>
                    {inscritos.map(inscrito=>{
                      const es=getEstadoStyle(inscrito.estado);
              ...
              <div style={{ display:'flex',alignItems:'center',gap:'12px' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:'12px',color:'#C2410C',margin:'2px 0 0',fontFamily:"'DM Sans', sans-serif" }}>
                    {pagosPendientes.length} pendiente{pagosPendientes.length!==1?'s':''} · Se confirman automáticamente vía webhook
                  </p>
                </div>
              </div>
            ...
            {pagosPendientes.length>0&&(
              <div style={{ padding:'12px 24px',background:'#EFF6FF',borderBottom:'1px solid #BFDBFE',flexShrink:0,display:'flex',alignItems:'flex-start',gap:'8px' }}>
                ...
                <span style={{ fontSize:'12px',color:'#1E40AF',fontFamily:"'DM Sans', sans-serif",lineHeight:1.4 }}>
                  Los pagos de Mercado Pago se confirman automáticamente. Solo confirma manualmente si verificaste el pago en tu panel de MP y el webhook no llegó.
                </span>
              </div>
            )}
            <div style={{ overflowY:'auto',flex:1 }}>
                      ...
                      <div key={pago.id} style={{ border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',overflow:'hidden',background:'#fff' }}>
                        <div style={{ padding:'16px 20px',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'16px',flexWrap:'wrap' }}>
                          ...
                          <div style={{ display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'6px',flexShrink:0 }}>
                            ...
                            {/* fontFamily único: monospace */}
                            <div style={{ fontSize:'11px',color:'var(--gray-400)',fontFamily:'monospace' }}>Ref: {pago.referencia}</div>
                          </div>
                        </div>
                        <div style={{ padding:'12px 20px',background:'var(--gray-50)',borderTop:'1px solid var(--border)',display:'flex',justifyContent:'flex-end',gap:'8px' }}>
                          <button onClick={()=>handleDeclinarPago(pago)} disabled={procesando}
                            ...
                            <XCircle style={{ width:'15px',height:'15px' }} />Declinar
                          </button>
                          <button onClick={()=>handleConfirmarPagoManual(pago)} disabled={procesando}
